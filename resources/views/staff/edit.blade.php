...
@section('title', 'Edit Ecosystem Profile')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1 font-medium">Update ecosystem credentials for {{ $staff->name }}.</p>
        </div>
    ...
    <form method="POST" action="{{ route('staff.update', $staff->id) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    <div class="relative group/photo cursor-pointer w-32 h-32 mx-auto md:mx-0">
                         <div class="w-32 h-32 rounded-2xl bg-gray-50 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 group-hover/photo:border-blue-400 group-hover/photo:text-blue-500 transition-all overflow-hidden">
                            @if($staff->profile_photo_path)
                                <img src="{{ Storage::url($staff->profile_photo_path) }}" class="w-full h-full object-cover" alt="Current Photo">
                            @else
                ...
                <div class="md:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="col-span-2 md:col-span-1">
                        ...
                        <input type="text" name="name" value="{{ old('name', $staff->name) }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium placeholder-gray-400">
                        @error('name') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                    </div>

                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-xs font-bold text-gray-700 mb-1.5">Email Address <span class="text-red-500">*</span></label>
                        <input type="email" name="email" value="{{ old('email', $staff->email) }}" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium placeholder-gray-400">
                        @error('email') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                    </div>

                    <div class="col-span-2 md:col-span-1">
                        <label class="block text-xs font-bold text-gray-700 mb-1.5">WhatsApp / Phone</label>
                        <input type="text" name="phone" value="{{ old('phone', $staff->phone) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium placeholder-gray-400">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">Age</label>
                            <input type="number" name="umur" value="{{ old('umur', $staff->umur) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium placeholder-gray-400">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1.5">City</label>
                            <input type="text" name="domisili" value="{{ old('domisili', $staff->domisili) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all outline-none font-medium placeholder-gray-400">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="relative">
                            <select name="university_id" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all outline-none font-medium cursor-pointer appearance-none">
                                ...
                                @foreach($universities as $university)
                                    <option value="{{ $university->id }}" {{ old('university_id', $staff->university_id) == $university->id ? 'selected' : '' }}>
                        ...
                        <div class="relative">
                            <select name="program_study" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all outline-none font-medium cursor-pointer appearance-none">
                                ...
                                @foreach($majors as $major)
                                    <option value="{{ $major }}" {{ old('program_study', $staff->program_study) == $major ? 'selected' : '' }}>
                                        {{ $major }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="github" value="{{ old('github', $staff->github) }}" class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all outline-none font-medium placeholder-gray-400">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="linkedin" value="{{ old('linkedin', $staff->linkedin) }}" class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all outline-none font-medium placeholder-gray-400">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden group hover:shadow-md transition-all duration-300">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="bank_id" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all outline-none font-medium text-sm cursor-pointer">
                            ...
                            @foreach($banks as $bank)
                                <option value="{{ $bank->id }}" {{ old('bank_id', $staff->bank_id) == $bank->id ? 'selected' : '' }}>
                                    {{ $bank->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                         ...
                         <input type="text" name="bank_account_number" value="{{ old('bank_account_number', $staff->bank_account_number) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all outline-none font-medium text-sm">
                    </div>
                    <div>
                         <label class="block text-xs font-bold text-gray-700 mb-1.5">Account Name</label>
                         <input type="text" name="bank_account_name" value="{{ old('bank_account_name', $staff->bank_account_name) }}" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all outline-none font-medium text-sm">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden group hover:shadow-md transition-all duration-300">
                ...
                <div class="flex items-center gap-4 mb-6 pb-4 border-b border-gray-50">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-500 font-medium">Update password if needed.</p>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                     <div>
                        ...
                        <input type="password" name="password" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all outline-none font-medium text-sm" placeholder="Leave blank to keep current">
                     ...
                     <div>
                        ...
                        <input type="password" name="password_confirmation" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all outline-none font-medium text-sm">
                     </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end pt-4">
             <button type="submit" class="inline-flex items-center gap-2 px-8 py-4 bg-black text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all text-sm group">
                ...
                <svg class="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
            </button>
        </div>
