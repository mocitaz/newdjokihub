...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="projectForm()">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Update project details, financials, and manage deliverables.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('projects.show', $project) }}" class="px-5 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold text-sm rounded-xl hover:bg-gray-50 transition-all">
                Cancel
            </a>
            <button type="button" @click="submitForm" class="px-5 py-2.5 bg-gray-900 text-white font-bold text-sm rounded-xl hover:bg-gray-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                Save Changes
            </button>
        </div>
    </div>

    <form id="editProjectForm" method="POST" action="{{ route('projects.update', $project) }}" class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        @csrf
        @method('PUT')
        <input type="hidden" name="deleted_deliverables" x-model="deletedIds">
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="col-span-full">
                         ...
                         <div>
                            ...
                            <input type="text" name="client_name" value="{{ old('client_name', $project->client_name) }}" placeholder="Leave blank for Anonymous"
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Order ID</label>
                            <input type="text" name="order_id" value="{{ old('order_id', $project->order_id) }}" 
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" 
                                  class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-medium text-gray-900 placeholder-gray-400 leading-relaxed">{{ old('description', $project->description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Status <span class="text-red-500">*</span></label>
                         <select name="status" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-medium text-gray-900">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="github_url" value="{{ old('github_url', $project->github_url) }}" placeholder="https://github.com/username/repo"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="url" name="google_drive_url" value="{{ old('google_drive_url', $project->google_drive_url) }}" placeholder="https://drive.google.com/drive/folders/..."
                                   class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all font-medium text-gray-900 placeholder-gray-400 text-sm">
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Add, edit, or mark items as completed.</p>
                    </div>
                ...
                <div class="p-6 space-y-3">
                    <template x-for="(item, index) in deliverables" :key="index">
                        <div class="flex items-start gap-3 group p-3 rounded-xl border border-transparent hover:border-gray-200 hover:bg-gray-50/50 transition-all">
                            <div class="flex-shrink-0 mt-3 flex items-center gap-2">
                                <div class="text-gray-300 font-mono text-xs select-none" x-text="(index + 1) + '.'"></div>
                                <input type="hidden" :name="'deliverables['+index+'][id]'" :value="item.id">
                                <input type="hidden" :name="'deliverables['+index+'][is_completed]'" :value="item.is_completed ? '1' : '0'">
                                <input type="checkbox" x-model="item.is_completed" class="w-4 h-4 text-green-600 rounded border-gray-300 focus:ring-green-500 focus:ring-offset-0 cursor-pointer" title="Mark as completed">
                            </div>

                            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                                <input type="text" :name="'deliverables['+index+'][name]'" x-model="item.name" placeholder="Item Name" required
                                       :class="item.is_completed ? 'text-gray-400 line-through' : 'text-gray-900'"
                                       class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm font-semibold placeholder-gray-400">
                                
                                <input type="text" :name="'deliverables['+index+'][description]'" x-model="item.description" placeholder="Description (Optional)"
                                       :class="item.is_completed ? 'text-gray-400 line-through' : 'text-gray-900'"
                                       class="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm font-medium placeholder-gray-400">
                            </div>
                    ...
                    <div x-show="deliverables.length === 0" class="text-center py-6 border-2 border-dashed border-gray-100 rounded-xl">
                        <p class="text-sm text-gray-500 font-medium">No deliverables found.</p>
                        <button type="button" @click="addDeliverable" class="mt-2 text-blue-600 hover:text-blue-700 text-xs font-bold hover:underline">Add your first item</button>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-gray-900 rounded-2xl p-6 text-white shadow-xl shadow-gray-200/50">
                ...
                <div class="space-y-5">
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-1.5">Total Budget</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-1.5">Admin Fee (%)</label>
                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide">Assign User</label>
            ...
            <div class="bg-yellow-50 rounded-2xl border border-yellow-100 shadow-sm p-6">
                ...
                <textarea name="notes" rows="4" placeholder="Any special instructions..."
                          class="w-full px-4 py-3 bg-white border border-yellow-200 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 outline-none transition-all font-medium text-gray-900 placeholder-gray-400 text-sm">{{ old('notes', $project->notes) }}</textarea>
            </div>
...
@php
    $formattedDeliverables = old('deliverables', $project->deliverables->map(function($d) {
        return [
            'id' => $d->id,
            'name' => $d->name,
...
function projectForm() {
    return {
        ...
        adminFee: {{ old('admin_fee_percentage', $project->admin_fee_percentage) }},
        nettBudget: {{ $project->nett_budget }},
        deliverables: @json($formattedDeliverables),
        deletedIds: '',
        isAvailable: {{ old('assigned_to') ? 'false' : ($project->assignees->count() > 0 ? 'false' : 'true') }},
        selectedStaff: @json(old('assigned_to', $project->assignees->pluck('id'))),
        ...
        addDeliverable() {
            this.deliverables.push({ name: '', description: '', is_completed: false });
        },
        
        removeDeliverable(index) {
            const item = this.deliverables[index];
            if (item.id) {
                // If it has an ID, track it for deletion
                const current = this.deletedIds ? this.deletedIds.split(',') : [];
                current.push(item.id);
                this.deletedIds = current.join(',');
            }
